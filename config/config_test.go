...
func TestUnmarshalJSON(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:      "errors on incomplete JSON",
			input:     `{"name":"test"`,
			expectErr: true,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			r := UnmarshalJSON[Config](ReaderOf(strings.NewReader(tc.input)))
			val, err := Read(context.Background(), r)
			if tc.expectErr {
				require.Error(t, err)
			} else {
...
func TestUnmarshalYAML(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			r := UnmarshalYAML[Config](ReaderOf(strings.NewReader(tc.input)))
			val, err := Read(context.Background(), r)
			if tc.expectErr {
				require.Error(t, err)
			} else {
