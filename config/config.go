...
// Package config provides abstractions for reading configuration values from various sources.
package config
...
func ValueOf[T any](val T) Value[T] {
	return Value[T]{val: val, set: true}
}
...
func Read[T any](ctx context.Context, reader Reader[T]) (T, error) {
	var zero T
...
func Must[T any](ctx context.Context, reader Reader[T]) T {
	val, err := Read(ctx, reader)
	if err != nil {
		panic(err)
	}
...
func MustOr[T any](ctx context.Context, defaultValue T, reader Reader[T]) T {
	return Must(ctx, Default(defaultValue, reader))
}
...
// UnmarshalJSON returns a Reader that decodes a JSON value from the given Reader.
func UnmarshalJSON[T any, R io.Reader](r Reader[R]) Reader[T] {
	return Map(r, func(ctx context.Context, reader R) (T, error) {
		var val T
...
// UnmarshalYAML returns a Reader that decodes a YAML value from the given Reader.
func UnmarshalYAML[T any, R io.Reader](r Reader[R]) Reader[T] {
	return Map(r, func(ctx context.Context, reader R) (T, error) {
		var val T
